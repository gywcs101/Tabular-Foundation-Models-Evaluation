...
- 主实验优先从 OpenML 选取 6 个代表性数据集，因为获取简单、可复现、适合两台核显笔记本。
- 实验设计参考 TALENT 和 TabArena，覆盖小数据、中等数据、较大数据、类别特征、缺失值、速度和可扩展性。
- 如果时间充裕，可从 TALENT 中补充 1 到 2 个数据集，但不把完整 TALENT 或 TabArena 全量复现作为目标。
...
2. 先跑通最小实验。
   使用 1 到 2 个小型 OpenML 数据集，确认数据读取、预处理、指标统计和模型调用流程可行。

3. 加入传统 baseline。
   使用 LightGBM 和/或 XGBoost，作为性能、速度、资源消耗的对照组。

4. 扩展到代表性 benchmark。
   从 OpenML 选取约 6 个代表性数据集，并参考 TALENT/TabArena 的评测思路覆盖不同样本数、不同特征类型、不同类别特征比例，观察模型随规模变化的表现。
...
- 数据集先从小中规模开始，不要一开始就选特别大的 benchmark。
- 速度和规模实验可以使用子采样，例如 500、1000、2000、5000、10000 条样本。
...
- OpenML 是优先实际取数平台；TALENT 和 TabArena 是重要评测参考，不要求全量复现。
