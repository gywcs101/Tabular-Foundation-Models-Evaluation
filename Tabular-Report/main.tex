...
\textbf{Time cost:} The more consequential divergence emerges in time cost. Figure 4 shows predict time on a log-log scale. TabICL achieves the highest Macro-F1 on the larger sample-scale datasets(sylvine, jm1) but is outperformed by TabPFN on pc1 and ringnorm and by LightGBM on kc1 at the smaller scales. This is captured quantitatively by the scalability exponent $\alpha$, estimated via log-log linear regression:
...
For the multi-class mfeat datasets, we examine confusion matrices to identify systematic error patterns. Table 6 lists the most frequent off-diagonal confusions across models.
