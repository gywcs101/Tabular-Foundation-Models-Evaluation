...
In this work, we conduct a systematic benchmark comparing two representative TFMs—TabPFN \cite{grinsztajn2025tabpfn} and TabICL \cite{tabiclv2_2026}—against two established gradient-boosted baselines—XGBoost and LightGBM—using 12 diverse classification datasets from the TALENT benchmark \cite{TALENT_benchmark}. All experiments are run under identical train/test splits. We evaluate these models across four dimensions:
...
The concept of a foundation model for tabular data was pioneered by TabPFN \cite{hollmann2023tabpfn}, a Transformer-based model pre-trained on millions of synthetic classification tasks generated from a prior over randomly initialized neural networks. By framing tabular prediction as an in-context learning problem, TabPFN can produce predictions for unseen datasets without any parameter updates, conditioning solely on a set of labeled context examples. TabPFN v2 \cite{hollmann2025tabpfnv2} scaled this approach with larger models, richer synthetic data generation priors, and improved context handling, demonstrating competitive or superior performance against tuned GBDTs on small-to-medium benchmarks. TabICL \cite{qu2025tabicl} extended the in-context learning paradigm to larger datasets by redesigning the context encoding mechanism to handle thousands of samples and features more efficiently, addressing the scalability bottleneck that limited earlier models. Subsequent works, including TabDPT \cite{ye2024tabdpt} and TabICLv2 \cite{qu2026tabiclv2}, have further explored directions such as real-data pre-training, retrieval-augmented context selection, and improved training strategies. Collectively, these models represent a shift from per-dataset training toward general-purpose tabular predictors that transfer knowledge across tasks.
...
Several benchmark platforms have been developed to standardize the evaluation of tabular machine learning methods. TALENT \cite{ye2024talent} provides a unified toolbox encompassing hundreds of classification and regression datasets, along with implementations of numerous deep learning and tree-based models. It organizes datasets by task type, sample size, and feature composition, making it a practical resource for controlled experiments. TabArena \cite{erickson2024tabarena} offers a continuously updated leaderboard that emphasizes fair comparison across model families, including standardized splits, consistent metric reporting, and computational cost analysis.
...
This study presents a systematic benchmark comparing two tabular foundation models—\text{TabPFN} and \text{TabICL}—against two established gradient-boosted baselines—\text{XGBoost} and \text{LightGBM}—across 12 \text{TALENT} classification datasets under a controlled-variable experimental design. Our evaluation spans four dimensions: predictive performance, computational cost, scalability, and confidence calibration.

\textbf{Predictive performance.} Both \text{TFMs} achieve superior average Macro-F1 compared to tree baselines, with \text{TabICL} leading at 0.851 (vs. \text{LightGBM} at 0.824). However, the advantage is not universal: on easy binary tasks all models perform similarly, and on severely imbalanced datasets (\texttt{jm1}, \texttt{default\_of\_credit\_card\_clients}) all struggle comparably. The \text{TFM} advantage is most meaningful on medium-complexity multi-class tasks such as the \texttt{mfeat} series.

\textbf{Computational cost.} The accuracy gains of \text{TFMs} come at a substantial price. On the largest dataset (30,000 samples), \text{TabPFN} predict time exceeds 4,000 seconds on a gaming laptop, while \text{LightGBM} completes inference in 22 milliseconds—a five-order-of-magnitude gap. \text{TabICL} is faster than \text{TabPFN} but still requires over 600 seconds on the same dataset. Peak memory consumption tells a similar story: \text{TabICL} reaches 18.8 GB on the largest dataset, while tree baselines remain under 250 MB throughout.

\textbf{Scalability.} The controlled-variable design reveals that \text{TFMs} scale substantially worse than tree baselines along both sample-size and feature-count dimensions. \text{TabPFN}'s predict-time scalability exponent $\alpha \approx 1.6$ indicates super-linear growth with sample size; \text{TabICL} is somewhat better at $\alpha \approx 1.4$, but both are far above the sub-linear scaling of \text{LightGBM} ($\alpha \approx 0.45$) and \text{XGBoost} ($\alpha \approx 0.37$). Feature-count scaling exhibits a similar pattern, though at lower absolute cost. Tree baselines are effectively insensitive to feature dimensionality in the tested range.
...
\textbf{Error patterns.} Cross-model analysis of confusion matrices reveals that the dominant error modes on \texttt{mfeat} datasets (e.g., 7$\leftrightarrow$10 digit confusions) are shared by all four model families and are driven by limited feature information rather than model architecture. As feature dimensionality increases, these errors largely disappear, confirming that richer representations benefit all models.

\textbf{Limitations.} This study has several scope constraints. First, we evaluate only classification tasks; regression, survival analysis, and other task types remain unexplored. Second, no hyperparameter tuning or fine-tuning is applied to any model, meaning that optimized versions of both \text{TFMs} and tree baselines could potentially yield different results. Third, while hardware sensitivity is assessed across two device classes, the machines are consumer laptops rather than laboratory-controlled hardware, so timing and memory results should be interpreted as practical runtime evidence rather than precise hardware benchmarks. Finally, the dataset selection, while carefully designed for controlled-variable analysis, is representative rather than exhaustive—conclusions may not generalize to all tabular tasks.

\textbf{Future work.} Several directions emerge from this study. Adding regression tasks and missing-value robustness experiments would provide a more complete picture of model capabilities. The retrieval-based in-context learning strategy, which selects context examples by similarity rather than randomly, offers a promising avenue to potentially improve \text{TFM} accuracy while reducing the number of context samples and associated inference cost. Evaluating these models on \text{GPU} hardware would further clarify whether the substantial \text{CPU}-time gap can be meaningfully narrowed with accelerator support.
